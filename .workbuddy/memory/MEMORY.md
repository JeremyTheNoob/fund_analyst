...
- 重构状态：核心框架已迁移，翻译层和基准构建已完成
  - ✅ `build_benchmark_ret()` - 已从原始代码迁移至 `data/fetcher.py`
  - ✅ `translate_results()` - 已在 `services/translator.py` 实现
  - ✅ **第二阶段功能 - Brinson归因、风格分析、滚动Beta（P1核心功能，2026-03-26已完成）**
  - ✅ **Alpha v2.0专业分层版（2026-03-26已完成）**
    - 周频数据转换（`resample_to_weekly` / `resample_to_monthly`）
    - 三层次Alpha计算引擎（CAPM → FF3/5 → 行业中性化）
    - Treynor-Mazuy择时检测（独立轨道，严禁与FF因子混用）
    - 月度Alpha胜率分析
  - ✅ **Alpha v2.0 P1+P2集成完成（2026-03-26）**
    - 集成到 `models/equity_model.py`
    - UI展示组件：`ui/alpha_v2_components.py`（分层Alpha/择时/胜率）
    - 完整测试验证通过
- 功能完整性：见 `重构功能完整性对比分析.md` 和 `数据源需求清单.md`
...
## 重构P1功能完成状态（2026-03-26）
- ✅ **`build_benchmark_ret()`** - 基准收益率构建（已从原始代码迁移至 `data/fetcher.py`）
- ✅ **`run_brinson()`** - Brinson归因分析（配置效应、选择效应、交互效应）
  - 实现文件：`models/brinson_analysis.py`
  - 核心功能：资产配置能力分析、选股能力分析、交互效应分析
  - 辅助函数：文字解读、雷达图评分
- ✅ **`analyze_style()`** - 风格分析（Beta、相关性、风格标签、风格细分）
  - 实现文件：`models/style_analysis.py`
  - 核心功能：Beta计算、相关性分析、风格判定、风险指标（跟踪误差、信息比率）
  - 风格细分：基于FF因子暴露（大小盘、价值成长、质量、动量）
- ✅ **`run_rolling_beta()`** - 滚动Beta监控（20/60日双窗口、风格漂移预警）
  - 实现文件：`models/style_analysis.py`
  - 核心功能：双窗口Beta计算、风格漂移检测、实时预警
  - 漂移检测：8种检测规则（偏离、转向、反转、趋势、波动等）
...
```
配置效应 = (基金股票权重 - 基准股票权重) × 基准股票收益率
         + (基金债券权重 - 基准债券权重) × 基准债券收益率
```

**选择效应**（Selection Effect）：
```
选择效应 = 基金股票权重 × (基金股票收益率 - 基准股票收益率)
         + 基金债券权重 × (基金债券收益率 - 基准债券收益率)
```

**交互效应**（Interaction Effect）：
```
交互效应 = (基金股票权重 - 基准股票权重) × (基金股票收益率 - 基准股票收益率)
         + (基金债券权重 - 基准债券权重) × (基金债券收益率 - 基准债券收益率)
```

**总超额收益**（Total Excess Return）：
```
...
### Beta计算公式
```
Beta = Cov(基金收益率, 基准收益率) / Var(基准收益率)
```

### 滚动Beta计算
```
滚动Beta(t) = Cov(基金收益率[t-window+1:t], 基准收益率[t-window+1:t]) / Var(基准收益率[t-window+1:t])
```

### 跟踪误差计算
```
跟踪误差 = Std(超额收益率) × √252
其中：超额收益率 = 基金收益率 - 基准收益率
```

### 信息比率计算
```
信息比率 = 年化超额收益率 / 跟踪误差
其中：年化超额收益率 = 超额收益率均值 × 252
```

### 风格漂移检测规则
- **⚠️ 风格剧烈漂移**：短期与长期Beta偏离>0.4
- **📊 风格明显漂移**：短期与长期Beta偏离>0.3
- **🛡️ 风格转向防御**：短期Beta<0.5，长期Beta>0.8
- **🚀 风格转向激进**：短期Beta>1.2，长期Beta<0.8
- **🔄 风格反转**：短期Beta从激进(<0.5)↔防御(>1.0)切换
- **📉 Beta持续下降**：60日滚动Beta过去10期中至少7期在下降
- **📈 Beta持续上升**：60日滚动Beta过去10期中至少7期在上升
- **⚠️ Beta波动剧烈**：20日滚动Beta标准差>0.3
...
| Beta>1.2 | - | 激进型（高Beta） |
| 0.8<=Beta<=1.2 | - | 平衡型 |
| 0.5<=Beta<0.8 | - | 防御型（低Beta） |
| Beta<0.5 | - | 低Beta（防御或避险） |
| - | \|r\|<0.3 | 低相关（独立风格） |
