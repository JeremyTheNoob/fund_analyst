...
def generate_idx_stock_report(
    ...
    if scale_str:
        try:
            ...
            if scale_val < 0.5:
                scale_warning = f"**规模预警**：当前规模 {scale_str}，低于 5000 万元清盘红线，存在清盘风险！"
    ...
    if is_etf:
        liquid_status = "流动性充足" if is_liquid else "流动性偏弱，买卖价差可能较大"
    ...
    if subtype == "passive":
        sec1_lines.extend([
            "",
            "### 跟踪精度",
            f"作为被动基金，核心任务就是精确复制指数。年化跟踪误差 {te_annual:.4f}%，",
            f"与基准相关系数 {corr:.4f}。",
    ...
    if ter <= 0.006:
        fee_assessment = f"TER {ter_annual_drag}%，比同类**便宜不少**，长期持有优势明显。"
    elif ter <= 0.010:
        fee_assessment = f"TER {ter_annual_drag}%，同类**中等水平**，不算贵也不算便宜。"
    elif ter <= 0.015:
        fee_assessment = f"TER {ter_annual_drag}%，偏**贵**了，长期复利下费率侵蚀可观。"
    else:
        fee_assessment = f"TER {ter_annual_drag}%，同类**偏贵**，每年白白吞噬 {ter_annual_drag}% 收益。"
    ...
    if subtype == "enhanced":
        ...
        sec2_lines.extend([
            "",
            f"**费率溢价**：相比典型被动型基金（TER 约 0.60%），",
            f"该增强型多收 {premium:.2f}% 费率。",
        ])
        if premium > 1.0:
            sec2_lines.append(
                f"溢价超过 1%，得看看超额收益（当前累计 {excess_ret:+.2f}%）"
                f"能不能覆盖这笔多出来的钱。年化超额得 > {premium:.2f}% 才划算。"
            )
        elif premium > 0.5:
            sec2_lines.append(
                f"溢价约 {premium:.2f}%，行业常见水平。"
                f"关键是看 IR={ir:.2f} 能不能持续稳定。"
            )
        else:
            sec2_lines.append(
                f"溢价才 {premium:.2f}%，增强策略的成本几乎可以忽略。"
            )

    sec2_lines.extend([
        "",
        f"**费率对收益的影响**：持有10年，{ter_annual_drag}% 的 TER 按 8% 年化算，",
    ...
    if pe_current is not None:
        sec3_lines.extend([
            ...
            f"近5年 **{pe_percentile:.1f}%** 分位（{pe_zone}）。",
    ...
    if pb_current is not None:
        ...
        sec3_lines.extend([
            ...
            f"近5年 **{pb_percentile:.1f}%** 分位（{pb_zone}）。",
    ...
    if subtype == "passive":
        ...
        if dividend_yield is not None:
            ...
            sec3_lines.extend([
                ...
                f"分红贡献了约 **{min(round(abs(dy/max(abs(ann_ret),0.01))*100, 1), 100):.1f}%**。",
                f"{'高股息指数的安全垫效应很实在。' if dy > 3 else '分红贡献有限，主要还是靠指数点位上涨。'}",
            ])
        sec3_lines.extend([
            "",
        ...
        # 增强型：IR + 因子暴露
        sec3_lines.extend([
            "增强基金赚的钱分两块：指数本身的涨跌（Beta）和基金经理多赚的（Alpha）。",
            "",
            f"1. **指数收益**（Beta）：{cum_bm:+.2f}% — 跟着标的指数被动赚的",
            f"2. **增强收益**（Alpha）：{excess_ret:+.2f}% — 基金经理主动操作多赚的",
            "",
            f"| 指标 | 数值 | 评价 |",
            f"|------|------|------|",
...
def _assess_tracking_quality(te: float, corr: float) -> str:
    ...
    if te <= 0.002 and corr >= 0.999:
        return f"复制得**非常精准**，TE 仅 {te:.4f}%，几乎就是指数的镜像。"
    elif te <= 0.005 and corr >= 0.998:
        return f"跟踪质量**不错**，TE {te:.4f}% 在同类中属于优秀水平。"
    elif te <= 0.010:
        return f"跟踪质量**一般**，TE {te:.4f}% 偏高，可能受现金拖累或管理费侵蚀影响。"
    else:
        return f"跟踪质量**较差**，TE {te:.4f}% 明显偏离，需排查是否频繁申赎导致。"
...
def _assess_enhanced_quality(ir: float, te: float, excess: float) -> str:
    ...
    parts = []
    if ir >= 1.5:
        parts.append(f"IR={ir:.2f}，**优秀水平**，增强策略长期有效。")
    elif ir >= 0.8:
        parts.append(f"IR={ir:.2f}，**良好水平**，增强有一定价值。")
    elif ir >= 0.3:
        parts.append(f"IR={ir:.2f}，**一般水平**，增强效果不太稳定。")
    elif ir >= 0:
        parts.append(f"IR={ir:.2f}，**偏低**，增强收益微弱，可能还不如直接买被动型。")
    else:
        parts.append(f"IR={ir:.2f}，**为负**，增强策略反而**拖累了收益**！")

    if excess < -2:
        parts.append(f"累计超额 {excess:+.2f}%，跑输基准，增强完全失效。")
...
def _analyze_alpha_persistence(charts: dict) -> List[str]:
    ...
    try:
        ...
        if win_rate >= 70:
            ...
            lines.append("Alpha 持续性**一般**，约一半月份有正贡献，时好时坏。")
        else:
            lines.append("Alpha 持续性**较差**，超额收益不稳定，可能有撞大运的嫌疑。")
...
def _assess_industry_concentration(conc: dict, index_name: str) -> str:
    ...
    # 基于 HHI 判断集中度
    if hhi > 800:
        lines.append(f"**高度集中**（HHI = {hhi:.0f}）：权重极度不均，")
        lines.append("少数巨头对指数走势有决定性影响，实质上成了头部股票指数。")
    elif hhi > 500:
        lines.append(f"**中度集中**（HHI = {hhi:.0f}）：前十大占比 {top10_sum:.1f}%，")
        lines.append("头部公司影响力较大，需留意单一行业风险暴露。")
    else:
        lines.append(f"**分散合理**（HHI = {hhi:.0f}）：权重分布较均匀，")
    ...
    if is_broad and top10_sum > 50:
        lines.append(
            f"\n作为**宽基指数**，前十大占比 {top10_sum:.1f}% 偏高，"
            f"需警惕行业集中风险。若某一行业占比超过 40%，"
            f"这个指数实际上更接近赛道指数。"
        )
...
def _build_passive_advice(
    ...
    if pe_zone in ("极度低估", "低估"):
        lines.extend([
            f"当前估值（{pe_zone}）是**不错的定投窗口**。",
            f"{index_name} 处于历史 {pe_pct:.0f}% 分位，安全边际较高。",
            "",
        ])
        if is_etf:
            if is_liquid:
                lines.append("**操作建议**：可通过场内ETF分批建仓，趁低估值慢慢攒筹码。")
            else:
                lines.append(
            ...
            lines.append("**操作建议**：通过基金公司官网或第三方平台定投，省心省力。")
    elif pe_zone in ("极度高估", "高估"):
        lines.extend([
            f"当前估值（{pe_zone}），**追高风险较大**。",
            f"{index_name} 处于历史 {pe_pct:.0f}% 分位。",
            "",
            "**操作建议**：已持有者可以继续拿着但暂停定投，等估值回来再说；没买的先观望。"
        ])
    else:
        lines.extend([
            f"当前估值（{pe_zone}），**中性区间**。",
            f"{index_name} 处于历史 {pe_pct:.0f}% 分位。",
            "",
            "**操作建议**：适合长期定投，不择时、不追涨、不杀跌，让时间复利发挥作用。",
        ])
    ...
    lines.extend([
        "",
        "### 选购要点",
        "- 买被动基金就是买便宜，优先选 TER 最低的同类产品",
    ])

    if te > 0.005:
        lines.append(f"- 该基金跟踪误差 {te:.4f}% 偏高，建议对比同类中 TE 更低的产品")

    if scale_warning:
        lines.append("- 规模低于清盘红线，不建议新资金进入")

    if is_etf and not is_liquid:
        lines.append("- ETF 流动性不足，优先考虑场外联接基金")
...
def _build_enhanced_advice(
    ...
    if ir >= 1.5 and excess > 5:
        lines.extend([
            ...
            "基金经理展现了持续的正 Alpha 能力。",
            "",
            "**操作建议**：可以替代同标的被动型，享受超额收益。",
        ])
        ...
        lines.extend([
            ...
            "",
            f"**操作建议**：费率溢价不大的话（当前 TER {ter*100:.2f}%）可以考虑；"
            "如果更看重确定性，被动型可能更省心。",
        ])
    elif ir >= 0 and excess >= 0:
        lines.extend([
            "**增强效果微弱，性价比存疑。**",
            f"信息比率 {ir:.2f}，累计超额仅 {excess:+.2f}%，可能连多出来的费率都覆盖不了。",
            "",
            "**操作建议**：建议对比同类被动型，看看多花的钱值不值。",
        ])
    else:
        lines.extend([
            "**增强策略失效，跑输基准了。**",
            f"累计超额 {excess:+.2f}%，IR={ir:.2f}，增强操作反而拖累了收益。",
            "",
            "**操作建议**：建议赎回或换成同标的被动型基金。",
        ])
    ...
    if pe_zone in ("极度低估", "低估"):
        lines.append(
            ...
            f"估值低，增强型在这个区间有更大的发挥空间。"
        )
    elif pe_zone in ("极度高估", "高估"):
        lines.append(
            f"当前 {index_name} 处于 {pe_zone}（{pe_pct_val:.0f}% 分位），"
            f"高估值环境下增强型的 Alpha 策略同样面临回撤风险。"
        )
    ...
    lines.extend([
        "",
        "### 持仓监控清单",
        f"- **IR 走势**：持续低于 0.3 要警惕策略失效",
        f"- **跟踪误差**：超过 2% 说明基金经理可能偏离了指数约束",
        f"- **超额收益回撤**：偶尔跑输正常，但连续6个月负 Alpha 就该关注了",
    ])
