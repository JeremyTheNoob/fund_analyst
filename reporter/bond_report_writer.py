...
def _section2_drawdown_recovery(
    ...
    if recovery_days > 0:
        ...
        recovery_text = (
            f"统计区间内尚未完全修复至前高，或回撤持续时间较短，需持续观察。"
        )
...
def _conclusion_advice(
    ...
    if calmar >= 5.0 and abs(max_dd) <= 0.5:
        manager_portrait = (
            f"该经理是典型的**「绝对收益导向型」**选手。"
            f"不追求极致的业绩排名，但极其厌恶回撤。"
            f"收益来源清晰：通过严控信用风险，换取确定性的票息溢价。"
        )
    elif calmar >= 2.0:
        manager_portrait = (
            f"该经理是**「稳健收益型」**选手。"
            f"在票息积累的基础上，适度参与久期博弈，"
            f"追求在不同债市环境下均能获取稳健的正收益。"
        )
    else:
        manager_portrait = (
            f"该经理风格偏向**「主动久期管理型」**，"
            f"在利率趋势判断上表现积极，收益弹性较大但回撤相对明显。"
        )
    ...
    if duration >= 5.0:
        ...
        risk_point = (
            f"由于其持仓偏向中短端且信用等级较高，"
            f"在债市大幅走牛（利率急剧下行）阶段，该基金可能因久期不足而**跟涨偏慢**。"
            f"这并非缺陷，而是稳健策略的必然代价。"
        )
    ...
    if grade in ("A+", "A"):
        allocation = (
            f"适合作为组合中的**「压舱石」或「现金替代品」**。"
            f"对于追求资产配置稳健性、对回撤容忍度极低的机构和个人投资者，"
            f"该基金具备极高的长期配置价值，"
            f"建议以 **20%~40%** 的仓位作为底仓长期持有。"
        )
    elif grade == "B":
        allocation = (
            f"适合有一定固收投资经验、追求略高于货币基金收益的投资者。"
            f"建议以 **10%~25%** 的比例配置，"
            f"并定期关注其久期变化和基金经理动向。"
        )
    else:
        allocation = (
            f"建议谨慎配置，密切跟踪回撤和久期变化，"
            f"待风格和数据企稳后再考虑增持。"
        )
